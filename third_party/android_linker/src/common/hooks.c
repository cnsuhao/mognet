...
static void* (*_android_dladdr)(void *addr, void *info) = NULL;
static int (*_android_dlclose)(void *handle) = NULL;
static const char* (*_android_dlerror)(void) = NULL;

// void* _android_dlopen(const char *filename, int flags);
// void* _android_dlsym(void* handle, const char *symbol);
// void* _android_dlvsym(void* handle, const char *symbol, const char* version);
// void* _android_dladdr(void *addr, void *info);
// int _android_dlclose(void *handle);
// const char* _android_dlerror(void);
...
FP_ATTRIB static int _hybris_hook_fprintf(FILE *fp, const char *fmt, ...)
{
    ...
    va_start(args, fmt);
    if(strcmp("%s : %s", fmt) == 0) {
        // TODO XXX STUFF MIGHT NOT PRINT!
        // ret = vfprintf(_get_actual_fp(fp), "%s: %s", args);
    } else {
...
static void* _hybris_hook_dladdr(void *addr, void *info)
{
    TRACE("addr %p info %p", addr, info);
    printf("_android_dladdr not implemented!\n");
    exit(1);
...
void* _hybris_hook___bss_start() {
  printf("_hybris_hook___bss_start\n");
  exit(1);
}
void* _hybris_hook___stack_chk_fail(void) {
  printf("\n\n_hybris_hook___stack_chk_fail\n\n");
  exit(1);
}
void* _hybris_hook___stack_chk_guard(void) {
  printf("\n\n_hybris_hook___stack_chk_guard\n\n");
  exit(1);
}
void* _hybris_hook___isfinite(void) {
  printf("\n\n_hybris_hook___isfinite\n\n");
  exit(1);
}
void* _hybris_hook___isfinitef(void) {
  printf("\n\n_hybris_hook___isfinitef\n\n");
  exit(1);
}
void* _hybris_hook___isinff(void) {
  printf("\n\n_hybris_hook___isinff\n\n");
  exit(1);
}
void* _hybris_hook___fpclassifyd(void) {
  printf("\n\n_hybris_hook___fpclassifyd\n\n");
  exit(1);
}
void* _hybris_hook___sF(void) {
  printf("\n\n_hybris_hook___sF\n\n");
  exit(1);
}
void* _hybris_hook__ctype_(void) {
  printf("\n\n_hybris_hook__ctype_\n\n");
  exit(1);
}
void* _hybris_hook__edata(void) {
  printf("\n\n_hybris_hook__edata\n\n");
  exit(1);
}
void* _hybris_hook__end(void) {
  printf("\n\n_hybris_hook__end\n\n");
  exit(1);
}
void* _hybris_hook__tolower_tab_(void) {
  printf("\n\n_hybris_hook__tolower_tab_\n\n");
  exit(1);
}
void* _hybris_hook__toupper_tab_(void) {
  printf("\n\n_hybris_hook__toupper_tab_\n\n");
  exit(1);
}
void* _hybris_hook___mmap2(void) {
  printf("\n\n_hybris_hook___mmap2\n\n");
  exit(1);
}

int ___isfinite(int x) {
    return __builtin_isfinite((float) x);
}

int ___isfinitef(float x) {
...
static struct _hook hooks_common[] = {
    HOOK_TO(__isfinite, ___isfinite),
    HOOK_TO(__isfinitef, ___isfinitef),
    HOOK_TO(__assert2, __assert),
    HOOK_TO(__mmap2, mmap),

    HOOK_DIRECT_NO_DEBUG(isalpha),
    HOOK_DIRECT_NO_DEBUG(isspace),
    HOOK_DIRECT_NO_DEBUG(isalnum),
    HOOK_DIRECT_NO_DEBUG(madvise),
    HOOK_DIRECT_NO_DEBUG(isprint),
    HOOK_DIRECT_NO_DEBUG(mktime),
    HOOK_DIRECT_NO_DEBUG(difftime),
    HOOK_DIRECT_NO_DEBUG(perror),
    HOOK_DIRECT_NO_DEBUG(syscall),
    HOOK_DIRECT_NO_DEBUG(write),
    HOOK_DIRECT_NO_DEBUG(time),
    HOOK_DIRECT_NO_DEBUG(lrand48),
    HOOK_DIRECT_NO_DEBUG(remove),
    HOOK_DIRECT_NO_DEBUG(getcwd),
    HOOK_DIRECT_NO_DEBUG(strtoul),
    HOOK_DIRECT_NO_DEBUG(strtoull),
    HOOK_DIRECT_NO_DEBUG(strtoll),
    HOOK_DIRECT_NO_DEBUG(read),
    HOOK_DIRECT_NO_DEBUG(rename),
    HOOK_DIRECT_NO_DEBUG(stat),
    HOOK_DIRECT_NO_DEBUG(mkdir),
    HOOK_DIRECT_NO_DEBUG(gethostname),
    HOOK_DIRECT_NO_DEBUG(unlink),
    HOOK_DIRECT_NO_DEBUG(rmdir),
    HOOK_DIRECT_NO_DEBUG(pread),
    HOOK_DIRECT_NO_DEBUG(tmpfile),
    HOOK_DIRECT_NO_DEBUG(mkstemp),
    HOOK_DIRECT_NO_DEBUG(pthread_sigmask),
    HOOK_DIRECT_NO_DEBUG(fnmatch),
    HOOK_DIRECT_NO_DEBUG(nanosleep),
    HOOK_DIRECT_NO_DEBUG(close),
    HOOK_DIRECT_NO_DEBUG(getrusage),
    HOOK_DIRECT_NO_DEBUG(__isinff),
    HOOK_DIRECT_NO_DEBUG(dl_iterate_phdr),
    HOOK_DIRECT_NO_DEBUG(wcscmp),
    HOOK_DIRECT_NO_DEBUG(fstat),
    HOOK_DIRECT_NO_DEBUG(__signbit),
    HOOK_INDIRECT(__stack_chk_fail),
    HOOK_DIRECT_NO_DEBUG(sched_yield),
    HOOK_INDIRECT(_tolower_tab_),
    HOOK_INDIRECT(_toupper_tab_),
    HOOK_DIRECT_NO_DEBUG(clock_gettime),
    HOOK_DIRECT_NO_DEBUG(gettimeofday),
    HOOK_DIRECT_NO_DEBUG(islower),
    HOOK_DIRECT_NO_DEBUG(isupper),
    HOOK_DIRECT_NO_DEBUG(tolower),
    HOOK_DIRECT_NO_DEBUG(toupper),
...
static void __hybris_linker_init()
{
    ...
    char path[PATH_MAX];
    const char *name = LINKER_NAME_JB; // XXX LINKER_NAME_DEFAULT
    ...
    const char *linker_dir = NULL; // LINKER_PLUGIN_DIR
    const char *user_linker_dir = getenv("HYBRIS_LINKER_DIR");
    if (user_linker_dir)
        linker_dir = user_linker_dir;

    printf("LINKER_PLUGIN_DIR undefined!\n");
    exit(1);
...
#undef WANT_ARM_TRACING
#ifdef WANT_ARM_TRACING
#error "WANT_ARM_TRACING not allowed in x86"
